...
const rn = new Date();
const date = `Logs-Universal-${minTwoDigits(rn.getMonth() + 1)}-${minTwoDigits(rn.getDate())}-${rn.getFullYear()}`;
  ...
  execute(client) {
    client.logger = createLogger({
      ...
      transports: [
        ...
        new transports.File({ filename: `Registros/Logs/${date}.log` }),
      ],
      rejectionHandlers: [
        new transports.Console(),
        new transports.File({ filename: `Registros/Logs/${date}.log` }),
    ...
    client.logger.info(`Bot Iniciado con Exito Dia de Registro ${String(new Date).split(" ", 5).join(" ")}`);
  },
};
