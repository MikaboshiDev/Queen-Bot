...
const { EmbedBuilder, AttachmentBuilder, Message, Client } = require("discord.js");
const { get } = require("request-promise-native");
const fs = require("fs");
const Pokemon = require("../../../Config/pokemon");
let scool = new Set();
let Guild = require("../../../Model/guild");
let User = require("../../../Model/user.js");
let levelUp = require("../../../Tools/Database/levelup");
let Spawn = require("../../../Model/spawn.js");
let shinyDb = require("../../../Tools/Database/shiny");
let Gen8 = require("../../../Tools/Database/gen8.js");
let color = 0x00ffff;

const common = fs
    .readFileSync("./Database/common.txt")
    .toString()
    .trim()
    .split("\n")
    .map((r) => r.trim());
const alolan = fs
    .readFileSync("./Database/alolans.txt")
    .toString()
    .trim()
    .split("\n")
    .map((r) => r.trim());
const mythic = fs
    .readFileSync("./Database/mythics.txt")
    .toString()
    .trim()
    .split("\n")
    .map((r) => r.trim());
const legend = fs
    .readFileSync("./Database/legends.txt")
    .toString()
    .trim()
    .split("\n")
    .map((r) => r.trim());
const ub = fs
    .readFileSync("./Database/ub.txt")
    .toString()
    .trim()
    .split("\n")
    .map((r) => r.trim());
const galarian = fs
    .readFileSync("./Database/galarians.txt")
    .toString()
    .trim()
    .split("\n")
    .map((r) => r.trim());
    ...
    await get(options)
        .then(async (t) => {
            ...
            let embed = new EmbedBuilder()
                .setTitle("Apareció un Pokémon salvaje")
                .setDescription(
                    `Adivina el Pokémon y el tipo <@679560282929889331> atrapar para atraparlo!`
                )
        ...
        .catch((err) => {
            if (err.message.includes(`404 - "Extraviado"`)) return; // channel.send(`Unable to spawn this pokemon due to no availability of this pokemon.\nName: ${random}`);
...
async function leveling(message, client) {
    ...
    let embed9 = new EmbedBuilder()
        .setTitle(`Felicidades ${message.author.username}!`)
        .setDescription(
            `Tu ${user.pokemons[selected].shiny ? "⭐" : ""} ${capitalize(
                user.pokemons[user.selected].name
            )} se ha nivelado a ${poke.level + 1}.`
        )
    ...
    if (newXp > neededXp) {
        ...
        for (var i = 0; i < levelUp.length; i++) {
            if (
                ...
                msg = `Felicidades ${message.author}! Tu \`${capitalize(
                    poke.name
                )}\` acaba de nivelarse a ${poke.level + 1
                    } y evolucionado hacia ${capitalize(levelUp[i].evo)}`;
